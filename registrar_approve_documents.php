...
if (!$hasRegistrarRole && !isSuperAdmin()) {
    die("<div class='text-center py-12'><p class='text-red-600 font-semibold'>Access Denied. Only Registrar admin can access this page.</p></div>");
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $action === 'approve' && $document_id) {
    ...
    if (!$document) {
        ...
        header('Location: registrar_approve_documents.php');
        exit;
    }
    
    if ($document['current_stage'] !== 'registrar' || $document['approval_status'] !== 'establishment_approved') {
    ...
    $update = $db->prepare("
        ...
        SET approval_status = 'registrar_approved',
    ...
    if ($update->execute([$admin_id, $approval_comments, $document_id])) {
        $notif_stmt = $db->prepare("
            ...
            VALUES (?, ?, 'registrar_approved', ?)
        ");
        $notif_msg = "Your document \"" . $document['original_filename'] . "\" has been approved by both Establishment and Registrar. Your document approval is complete.";
        ...
        if ($user_info && !empty($user_info['official_email'])) {
            ...
            $email_service->sendDocumentApprovedEmail(
        ...
        $_SESSION['success'] = 'Document approved. User has been notified via email.';
    } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $action === 'reject' && $document_id) {
    ...
    if (empty($rejection_reason)) {
        ...
        header('Location: registrar_approve_documents.php?action=reject&id=' . $document_id);
        exit;
    }
    ...
    if ($update->execute([$rejection_reason, $rejection_comments, $admin_id, $document_id])) {
        $notif_stmt = $db->prepare("
            ...
            VALUES (?, ?, 'rejected', ?)
        ");
        $notif_msg = "Your document \"" . $document['original_filename'] . "\" has been rejected by the Registrar. Reason: " . $rejection_reason;
        $notif_stmt->execute([$document['admin_id'], $document_id, $notif_msg]);
...
$pageTitle = 'Document Approvals (Registrar)';
...
$submissions = $db->fetchAll("
    ...
    WHERE ds.current_stage = 'registrar' AND ds.approval_status = 'establishment_approved' $where_search
    ...
    <div class="max-w-4xl mx-auto">
        <div class="card mb-6">
            <div class="flex justify-between items-center mb-6">
                ...
                <a href="registrar_approve_documents.php" class="btn-secondary">
            ...
            <div class="grid grid-cols-2 gap-4 mb-6 pb-6 border-b">
                ...
                <div>
                    <p class="text-gray-600 text-sm">Approved By (Establishment)</p>
                    <p class="font-semibold"><?php echo htmlspecialchars($document['est_firstname'] . ' ' . $document['est_surname']); ?></p>
                    <p class="text-gray-700 text-sm"><?php echo date('M d, Y H:i', strtotime($document['establishment_approved_at'])); ?></p>
                </div>
            </div>
            ...
            <form method="POST" action="?action=approve&id=<?php echo $document_id; ?>" class="mb-6">
                ...
                <div class="flex gap-4">
                    <button type="submit" class="btn-primary" onclick="return confirm('Are you sure you want to approve this document? The user will be notified.');">
                        <i class="fas fa-check mr-2"></i>Approve Document
                    </button>
                    <a href="registrar_approve_documents.php?action=reject&id=<?php echo $document_id; ?>" class="btn-danger">
        ...
        <div class="card">
            ...
            <form method="POST" action="?action=reject&id=<?php echo $document_id; ?>">
                ...
                <div class="flex gap-4">
                    ...
                    <a href="registrar_approve_documents.php" class="btn-secondary">Cancel</a>
                </div>
            </form>
        </div>
    </div>
        ...
        <div class="card mb-6">
            <div class="mb-6">
                <h2 class="text-2xl font-bold text-gray-800 mb-2">
                    <i class="fas fa-file-check text-blue-600 mr-2"></i>Document Approvals (Registrar)
                </h2>
                <p class="text-gray-600">Review and approve documents from Establishment</p>
            </div>
                    ...
                    <table class="w-full">
                        <thead>
                            <tr class="bg-gray-100 border-b">
                                ...
                                <th class="px-4 py-3 text-left">Approved By (Est.)</th>
                        ...
                        <tbody>
                            <?php foreach ($submissions as $doc): ?>
                                <tr class="border-b hover:bg-gray-50">
                                    ...
                                    <td class="px-4 py-3">
                                        <?php echo htmlspecialchars($doc['est_firstname'] . ' ' . $doc['est_surname']); ?>
                                        <br><small class="text-gray-600"><?php echo date('M d, Y', strtotime($doc['establishment_approved_at'])); ?></small>
                                    </td>
